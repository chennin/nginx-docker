...
REV=$(grep -oP '\d' argfile.conf | awk '{s+=$1} END {print s}')
CONT_VER=$(buildah --storage-driver $STORAGE_DRIVER run version-finder sh -c "nginx -V 2>&1 | grep -oP '(?<=nginx version: nginx/)(.+)$'")-${REV}
CONT_VER=${CONT_VER//[+~]/_}
